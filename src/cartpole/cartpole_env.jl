...
mutable struct CartpoleEnvironment{M<:Mechanism,S<:MechanismState,I} <: RigidBodyRLEnvironment
    mechansim::M
    state::S
    integrator::I
...
function CartpoleEnvironment(;
    ...
    gravity = Float64(g)
    CartpoleEnvironment(cart_mass, pole_mass, pole_length, pole_com, xlimits, dt, simstep, gravity)
end
...
function CartpoleEnvironment(
    ...
    env = CartpoleEnvironment{typeof(mechanism),typeof(state),typeof(integrator)}(
        mechanism,
        state,
        integrator,
        torques,
...
function resetenv!(rng::AbstractRNG, env::CartpoleEnvironment, s0 = SVector(0.,0.,0.,0.))
    configuration(env.state)[1] = Float64(s0[1])
    configuration(env.state)[2] = Float64(s0[3])
    velocity(env.state)[1] = Float64(s0[2])
    velocity(env.state)[2] = Float64(s0[4])
    setdirty!(env.state)
    env.time = 0.
    env.torques .= 0.
    env.state
...
function stepenv!(rng::AbstractRNG, env::CartpoleEnvironment, force::Number = 0.)
    steps = floor(Int, env.dt / env.simstep)
    for i in 1:steps
        @inbounds env.torques[1] = Float64(force)
        OdeIntegrators.step(env.integrator, env.time, env.state, env.simstep)
        env.time += env.simstep
    end
    env.state
end
