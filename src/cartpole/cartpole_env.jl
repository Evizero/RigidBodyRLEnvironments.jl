struct CartpoleEnvironment{E<:CartpoleSimulator,F,R,I,T} <: Environment
    sim::E
...
function cartpole_statefun(rng, env::CartpoleEnvironment, simstate)
...
function CartpoleEnvironment(;
        ...
        rewardfun = (rng, env, a, simstate) -> 1,
        initfun =  (rng, env) -> (0.1 .* rand(rng,SVector{4}) .- 0.05),
        isterminalfun = cartpole_isterminalfun(pos_limit, angle_limit)
       )
    sim = CartpoleSimulator(M, m, l, com, (-pos_limit, pos_limit), dt)
    CartpoleEnvironment(sim, convert(Vector{Float64}, actions), Float64(discount), statefun, rewardfun, initfun, isterminalfun)
end
...
function reset!(rng::AbstractRNG, env::CartpoleEnvironment)
    simstate = reset!(rng, env.sim, env.initfun(rng, env))
    env.statefun(rng, env, simstate)
end

function step!(rng::AbstractRNG, env::CartpoleEnvironment, a)
    simstate = step!(env.sim, a)
    env.statefun(rng, env, simstate), env.rewardfun(rng, env, a, simstate)
end

function POMDPs.isterminal(env::CartpoleEnvironment, s)
    env.isterminalfun(env, s)
end
